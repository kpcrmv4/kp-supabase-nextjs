...
| Skill | Covers |
|-------|--------|
| `nextjs-supabase-ssr-auth` | @supabase/ssr four clients, proxy/middleware gate (`getClaims()`), rate-limited PIN login, new publishable/secret keys, and the four gate bugs (API→/login redirect; sign-in cookie must bind to the response; middleware↔login loops; matcher 307ing sw.js/manifest kills PWA install+push logged-out) |
